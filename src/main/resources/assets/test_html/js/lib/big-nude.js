...
scanImage = function(){
  ...
  var result = nudeScanImage(imageData, canvas.width, canvas.height);
  return result;
...
//   nudeCanvas.width = event.data[1];
//   nudeCanvas.height = event.data[2];
...
function nudeScanImage(imageData, width, height){

  skinRegions = [];
  skinMap = [];
  nudeCanvas = {};

  nudeCanvas.width = width;
  nudeCanvas.height = height;

  var detectedRegions = [],
    mergeRegions = [],
    width = nudeCanvas.width,
    lastFrom = -1,
    lastTo = -1;
...
// sort the detected regions by size
  var length = skinRegions.length,
    totalPixels = nudeCanvas.width * nudeCanvas.height,
    totalSkin = 0;
